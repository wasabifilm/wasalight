...
WASALIGHT_INSTALL_COMMIT=${WASALIGHT_INSTALL_COMMIT:-$(git -C "$PROJECT_DIR" rev-parse --verify \
...
if grep -Eq '__WASALIGHT_(INSTALL_VARIANT|NETWORK_PRELOAD|INSTALLER_VERSION|BOOTSTRAP_PACKAGES)__|/cdrom/wasalight' "$TMP/autoinstall.yaml"; then
