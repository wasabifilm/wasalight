...
class ApplicationShell(Gtk.Box):
    ...
    def configure_language_menu(self, save_language):
        """Keep the Control language outside the operational page hierarchy."""
        button = Gtk.MenuButton()
        button.get_style_context().add_class("language-button")
        button.set_size_request(-1, 52)
        button_box = Gtk.Box(spacing=10)
        button_box.pack_start(
            image_for("preferences-desktop-locale", 22), False, False, 0)
        language_name = {
            "auto": _("Automatic"), "it": "Italiano", "en": "English",
        }.get(current_language(), _("Automatic"))
        button_label = Gtk.Label(label=f"{_('Language')}  ·  {language_name}")
        button_label.set_xalign(0)
        button_box.pack_start(button_label, True, True, 0)
        button.add(button_box)

        popover = Gtk.Popover.new(button)
        popover.get_style_context().add_class("popover-card")
        content = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=12)
        content.set_border_width(16)
        title = Gtk.Label()
        title.set_xalign(0)
        title.set_markup(
            f"<span size='14000' weight='bold'>{GLib.markup_escape_text(_('Language'))}</span>")
        content.pack_start(title, False, False, 0)
        description = Gtk.Label(
            label=_("Applied the next time Wasalight Control starts."))
        ...
        save.connect("clicked", save_language, chooser)
        content.pack_start(save, False, False, 0)
        popover.add(content)
